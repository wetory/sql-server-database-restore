/*  
	...
	Description: Script is creating standardized maintenance solution for SQL Server instances under APPS_SO_REGION SKÅNE_CZ responsibility. 
	...
	Author: Tomas Rybnicky (tomas.rybnicky@tieto.com)
	Date of last update: 
	 v1.9.1 - 03.08.2017 - bug with used 3402 traceflag fixed (fix script can be found https://drive.google.com/open?id=0B2ywpnaeyEUFMFQ0LXZaSnMxUFE)
...
SET NOCOUNT ON
GO
DECLARE @ScriptVersion			NVARCHAR(16) = '1.9'
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
	  ...
	  -- this is modified so as to fit our folder structures - tomas.rybnicky@tieto.com
      INSERT INTO @CurrentDirectories (ID, DirectoryPath, Mirror, DirectoryNumber, CreateCompleted, CleanupCompleted)
      SELECT 
		ROW_NUMBER() OVER (ORDER BY ID), 
		DirectoryPath + 
		CASE 
			WHEN RIGHT(DirectoryPath,1) = '\' THEN '' 
			ELSE '\' 
		END, 
		Mirror, 
		ROW_NUMBER() OVER (PARTITION BY Mirror ORDER BY ID ASC), 0, 0
      FROM @Directories
      ORDER BY ID ASC
      ...
      BEGIN
        ...
        BEGIN
          ...
          SET @CurrentFilePath = @CurrentDirectoryPath + @CurrentDatabaseNameFS + CASE WHEN @ReadWriteFileGroups = 'Y' THEN '_PARTIAL' ELSE '' END + CASE WHEN @CopyOnly = 'Y' THEN '_COPY_ONLY' ELSE '' END + '_' + REPLACE(REPLACE(REPLACE((CONVERT(nvarchar,@CurrentDate,120)),'-',''),' ','_'),':','') + CASE WHEN @NumberOfFiles > 1 AND @NumberOfFiles <= 9 THEN '_' + CAST(@CurrentFileNumber AS nvarchar) WHEN @NumberOfFiles >= 10 THEN '_' + RIGHT('0' + CAST(@CurrentFileNumber AS nvarchar),2) ELSE '' END + '.' + @CurrentFileExtension
      ...
      IF @URL IS NOT NULL
      BEGIN
        SET @CurrentFilePath = @URL + CASE WHEN RIGHT(@URL,1) = '/' THEN '' ELSE '/' END + CASE WHEN @ReadWriteFileGroups = 'Y' THEN '_PARTIAL' ELSE '' END + CASE WHEN @CopyOnly = 'Y' THEN '_COPY_ONLY' ELSE '' END + '_' + REPLACE(REPLACE(REPLACE((CONVERT(nvarchar,@CurrentDate,120)),'-',''),' ','_'),':','') + '.' + @CurrentFileExtension

        IF LEN(@CurrentFilePath) > 259
        BEGIN
          SET @CurrentFilePath = @URL + CASE WHEN RIGHT(@URL,1) = '/' THEN '' ELSE '/' END + @CurrentDatabaseNameFS + CASE WHEN @ReadWriteFileGroups = 'Y' THEN '_PARTIAL' ELSE '' END + CASE WHEN @CopyOnly = 'Y' THEN '_COPY_ONLY' ELSE '' END + '_' + REPLACE(REPLACE(REPLACE((CONVERT(nvarchar,@CurrentDate,120)),'-',''),' ','_'),':','') + '.' + @CurrentFileExtension
        END
...
BEGIN
	...
	SET @BackupDirectory = (SELECT Value FROM #Config WHERE Name = 'DefaultBackupDirectory')
	SET @LogToTable = (SELECT Value FROM #Config WHERE Name = 'LogToTable')
	SET @JobDescription = (SELECT Value FROM #Config WHERE Name = 'JobDescription')
	...
	SET @JobName						= N'Maintenance_FullBackup'
	SET @OutputFileName = @OutputFileDirectory + '\' + 'FullBackup_step_' + @TokenStepID + '_' + @TokenDate + '_' + @TokenTime + '.txt'
...
@Directory = ''' + @BackupDirectory + '\System\'',
@BackupType = ''FULL'',
@Verify = ''N'',
@CheckSum = ''Y'',
...
@Directory = ''' + @BackupDirectory + '\User\Data\'',
@BackupType = ''FULL'',
@Verify = ''N'',
@CheckSum = ''Y'',
@LogToTable = ''' + @LogToTable + ''',
...
DBCC TRACEOFF  (3042,-1);',
	...
	SET @JobName						= N'Maintenance_DiffBackup'	
	SET @OutputFileName = @OutputFileDirectory + '\' + 'DiffBackup_step_' + @TokenStepID + '_' + @TokenDate + '_' + @TokenTime + '.txt'
	EXEC [master].[dbo].StdMaintenanceCreateJob 
		@JobName						= @JobName,	
		@JobDescription					= @JobDescription,   
		@ScheduleName					= N'Weekly',
...
@CleanupTime = 168'
	...
	BEGIN		
		...
		BEGIN		
			SET @TempCommandVar = N'EXECUTE dbo.DatabaseBackup
@Databases = ''USER_DATABASES, -BAMPrimaryImport%, -BAMAlertsApplication%, -BAMAlertsNSMain%, -BizTalkDTADb%, -BizTalkMgmtDb%, -BizTalkMsgBoxDb%, -BizTalkRuleEngineDb%, -SSODB%, -BAMStarSchema%, -BAMAnalysis%, -BAMArchive%, -BizTalkAnalysisDb%'',
@Directory = ''' + @BackupDirectory + '\User\Differential\'',
@BackupType = ''DIFF'',
@ChangeBackupType = ''Y'',
@Verify = ''N'',
@CheckSum = ''Y'',
...
@PhysicalOnly = ''Y'''
	...
	SET @JobName						= N'Maintenance_TlogBackup'	
	SET @OutputFileName = @OutputFileDirectory + '\' + 'TlogBackup_step_' + @TokenStepID + '_' + @TokenDate + '_' + @TokenTime + '.txt'
	EXEC [master].[dbo].StdMaintenanceCreateJob 
		@JobName						= @JobName,	
		@JobDescription					= @JobDescription,   
...
@CleanupTime = 168'					
	...
	BEGIN
		...
		BEGIN		
			SET @TempCommandVar = N'EXECUTE dbo.DatabaseBackup
@Databases = ''USER_DATABASES, -BAMPrimaryImport%, -BAMAlertsApplication%, -BAMAlertsNSMain%, -BizTalkDTADb%, -BizTalkMgmtDb%, -BizTalkMsgBoxDb%, -BizTalkRuleEngineDb%, -SSODB%, -BAMStarSchema%, -BAMAnalysis%, -BAMArchive%, -BizTalkAnalysisDb%'',
@Directory = ''' + @BackupDirectory + '\User\Translog\'',
@BackupType = ''LOG'',
@ChangeBackupType = ''Y'',
@Verify = ''N'',
@CheckSum = ''Y'',
